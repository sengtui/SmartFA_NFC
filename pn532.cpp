...
int PN532::rawCommand(int txLen, int isLog)
{
    ...
    try{
    // Send Command
        if(isLog>ON_EVENT) printHEX("TX", txStr, txLen);
...
// Read heading: 0x00 0x00 0xFF LEN SUM
        ...
        for(int i=0;i<ret-3;i++){
            if((unsigned char)rxStr[i]==255){
                ...
                if((LEN+LCS)==0){
                    CMD=i+3;
                    if(ret<CMD+LEN+2){
                        if(isLog>=ON_EVENT) fprintf(stderr,"Length %d, expected %d (CMD %d + LEN %d + 2)\n", ret, (CMD+LEN+2), CMD, LEN);
                        if(dev->dataAvailable(10)) ret+=dev->read(rxStr+ret, (CMD+LEN+2-ret));
                        if(ret<CMD+LEN+2){
                            if(dev->dataAvailable(10)) ret+=dev->read(rxStr+ret, (CMD+LEN+2-ret));
                        }
                    }
        ...
        if(isLog>ON_EVENT) printHEX("HEAD", rxStr, ret);

        if(isLog>ON_EVENT) cout<< "LEN: " << LEN <<" LCS: "<< LCS << endl;
